...
关于 14:05 送达：GitHub 的 `schedule` 定时任务存在排队延迟，实测本仓库通常晚约 40 分钟。为让微信推送尽量在 14:05 前后到达，工作流已提前到北京时间 13:25（UTC 05:25）触发以补偿排队延迟，实测送达时间会有几分钟到十几分钟的浮动。若需要精确 14:00-14:10 触发，可把工作流改为 `workflow_dispatch` 手动触发，并用 cron-job.org 等定时服务在 14:05 调用 GitHub API 触发（需要额外的 PAT 密钥，配置更复杂），本项目暂以提前触发的补偿方案为主。
